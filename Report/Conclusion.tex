...
Respecto al protocolo de Nintendo para juegos en línea en \acl{NDS} (sección~\ref{sec:mp-nintendo-server}), ofrece mecanismos de autenticación usando \texttt{CHAP} y cifrando la comunicación en \texttt{HTTPS}.
...
Sin embargo, la forma de autenticación es mediante una contraseña incluida en texto plano en el juego (mecanismo~\texttt{PAP}).
...
Cifrando la base de datos o incluso comprobando que el usuario había realizado dicha compra se podría evitar este caso.
...
Sin embargo, si se modificaba el juego de original, en la segunda consola se podía ejecutar código cambiado también.
...
Los estudios de este trabajo han pretendido resumir los mecanismos más frecuentes encontrados en los juegos de \ac{NDS} así como, señalas las carencias de las plataformas móviles.
