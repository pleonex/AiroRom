...
Esta fuente menciona también la ingeníera inversa como `\textit{usando herramientas genéricas, los hackers pueden convertir rápidamente binarios desprotegidos en código fuente, volver a empaquetarlos y distribuirlos}'.
...
\foreignquote{spanish}{Hacker: Persona apasionada por entender cómo funciona internamente y en detalle, un conjunto de sistemas, ordenadores y redes de ordenadores. Generalmente se usa de forma incorrecta en un contexto peyorativo, en este caso el término correcto sería `cracker'.}
...
En los siguientes apartados se introduce los más comunes, tratados a lo largo del proyecto.
...
El cifrado simétrico hace referencia a que existe una misma clave para cifrar y descifrar.
...
Sin embargo no hacen falta aplicar operaciones tan complejas cuando solo se desea ofuscar el texto como sucede en los videojuegos.
...
Los algoritmos deben de cumplir que las colisiones (coincidencias de resumen entre dos bloques de datos distintos) sean no reproducibles.
...
Debido a la existencia de tarjetas MicroSD, se pudo crear dispositivos que simulan ser un juego comercial y que, permiten ejecutar código no autorizado por Nintendo.
...
Sin embargo, esos caracteres están protegidos con copyright por Sega y en base a esto, creó una demanda.
...
El formato de los juegos se mantiene pero en aquellos juegos exclusivos para la nueva generaciónm se añade una firma digital usando la clave privada de Nintendo.
...
La siguiente generación de consolas de Nintendo aumentó aún más la seguridad.
...
Se trata de un módulo diseñado específicamente para la consola y la clave está sobre las pistas del chip por lo que no se puede averiguar.
...
Un proceso manual que permitió encontrar una serie de \textit{exploits}\footnote{\url{http://smealum.net/ninjhax/}} y aprovechándose de nuevo, de fallos de seguridad en los archivos de guardado, para ejecutar código descifrado directamente.
...
Este proyecto identificó dos tareas principales a la hora de realizar un trabajo de ingeníera inversa~\cite[p.~17]{REDO}:
...
\begin{itemize}
    ...
    Esta tarea se ha llevado documentando los algoritmos estudiados.
\end{itemize}
...
Este proceso consiste en transformar el código de forma que es menos leíble, pero mantiene la funcionalidad~\cite[p.~344]{RevBlack}.
...
Esta se centra más sobre los recursos y sus formatos que en el código y funcionalidad\footnote{En la siguiente dirección se encuentra una guía de introducción a la ingenería inversa para \acl{NDS}:

\url{http://gbatemp.net/threads/gbatemp-rom-hacking-documentation-project-new-2014-edition-out.73394/}}.

\subsection{Legalidad}
La ingeniería inversa es el proceso de examinar, estudiar, comprender y documentar~\cite[p.~17]{REDO}.
