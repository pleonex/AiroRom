...
\label{sec:art-sec}
En la Sección~\ref{sec:int-sec} se comentan los aspectos de seguridad que se buscan solventar.
En las siguiente secciones se explicarán expondrán algunos algoritmos de uso común y a los que se hará referencia a lo largo del trabajo.

\subsection{Cifrado simétrico y asimétrico}
El cifrado es un técnica para garantizar la confidencialidad.
Mediante un algoritmo, un bloque de datos original se transforma en otro, reversible, pero que a simple vista parece aleatorio.
Existen dos tipos de cifrados: simétrico y asimétrico.

% TODO: Leer SRC y meter rollo de estandares y tal e imagenes y tal

El cifrado simétrico hace referencia a que existe una misma clave tanto para cifrar como para descifrar.
Esta clave ha de conocerse en los dos extremos para poder establecer una comunicación con éxito.
El algoritmo más común en este tipo de cifrado es la operación \texttt{XOR} sobre cada byte.
Esta operación tiene la particularidad de cumplir \verb!a XOR b = c; b XOR c = a!, de forma que siendo respectivamente, el byte descifrado, la clave y el byte cifrado, se puede revertir este aplicando la operación XOR de nuevo.
Sin embargo, en determinadas situaciones esta propiedad presenta un fallo de seguridad, pues si el byte descifrado es cero, el byte cifrado sería igual a la clave, que se transmitiría o almacenaría.

% TODO: Añadir tabla de XOR

El cifrado asimétrico, a pesar de ser más lento que el simétrico, mejora la seguridad al utilizar claves diferentes para descifrar y cifrar.
...
El algoritmo más utilizado es \texttt{RSA}.
% TODO: Meter historia y conceptos basicos de SRC
...
% TODO: Meter rollo de SRC.

\subsection{Firma digital}
La firma digital es una técnica para proveer tanto autenticidad como integridad sobre el mensaje.
Consiste en aplicar un algoritmo de integridad sobre un bloque de datos (integridad) y al resultado cifrarlo con la clave privada del emisor (autencidad).
De esta forma, solo la clave pública podrá descifrar el resultado del algoritmo \textit{hash}, por lo que se asegura que ha sido esa persona quien ha realizado la operación, y comprobar que el mensaje no ha sido modificado.
El algoritmo emplea \texttt{SHA-1} para la integridad y \texttt{RSA} para el cifrado.
% TODO: Meter rollo de SRC.
