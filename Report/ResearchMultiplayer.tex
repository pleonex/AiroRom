...
\section{Lugar de la seguridad}
Existen ocasiones donde la parte de seguridad se implementa del lado de la aplicación, videojuego, y no del servidor. Ya sean motivos técnicos o económicos para evitar una sobrecarga del servidor o poseer uno dedicado, esto deja una brecha permanente. Una vez estudiada la misma, sería tan fácil como editar el juego para saltarse el mecanismo.

Un ejemplo a lo expuesto se encuentra en el juego de \textit{El Profesor Layton - London Life}. Como se ha descrito en la sección % TODO: añadir referencia
los textos de este juego se encuentran comprimidos en el archivo \textit{msg.dat} con un formato propietario no estándar. De esta manera se dificulta tanto su acceso y aun más su modificación pues habría que estudiar el algoritmo y crear los programas convenientes. Sin embargo, es algo factible.

El juego tiene una opción de conectividad on-line.
El problema que conlleva la modificación de los textos es que también se incluyen las expresiones regulares que determinan si el nombre que ha introducido el usuario cuando se conecta al modo on-line está permitido o no. De esta forma se evita dar con jugadores con nombres ofensivos.
